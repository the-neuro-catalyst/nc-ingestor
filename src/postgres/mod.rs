...
use tokio_postgres::{Config as TokioPgConfig, NoTls, CopyInSink};
use tracing::info;
...
impl Ingestor for PostgresIngestor {
    async fn new(config: IngestorConfig,) -> Result<Self,> {
        ...
        execute_with_retry(|| async {
            pool.get().await
                .map(|_| ())
                .map_err(|e| wrap_error(IngestorError::ConnectionError(format!("Failed to get client from pool: {}", e))))
        }).await?;
    ...
    async fn ingest(&self, data: DataReaderResult,) -> Result<(),> {
        ...
        match data {
            DataReaderResult::Csv(csv_data, _metadata,) => {
                if let Some(schema) = csv_data.inferred_schema {
                    let client = self.pool.get().await.map_err(|e| IngestorError::ConnectionError(e.to_string()))?;
                    ...
                    self.ingest_via_copy(csv_data.nc_rows.into_iter(), &table_name, &col_names, mappings).await?;
                } else {
                    self.ingest_as_blob(DataReaderResult::Csv(csv_data, _metadata), &table_name).await?;
                }
            },
        ...
        info!("Successfully ingested data to PostgreSQL table '{}'.", table_name);
        Ok(())
    }
}
...
impl PostgresIngestor {
    async fn ingest_via_copy(
        ...
        let mapped_cols: Vec<String> = col_names.iter().map(|c| {
            let target = mappings.as_ref().and_then(|m: &HashMap<String, String>| m.get(c)).unwrap_or(c);
            format!("\"{}\"", target)
        }).collect();

        let copy_query = format!("COPY \"{}\" ({}) FROM STDIN (FORMAT CSV, HEADER FALSE)", table_name, mapped_cols.join(", "));
        
        let sink: CopyInSink<Bytes> = client.copy_in(&copy_query).await.map_err(|e| IngestorError::DatabaseError(e.to_string()))?;
        pin_mut!(sink);
        ...
        for row in rows {
            if let serde_json::Value::Object(obj) = row {
                ...
                line.push('\n');
                sink.send(Bytes::from(line)).await.map_err(|e: tokio_postgres::Error| IngestorError::IngestionError(e.to_string()))?;
            }
        }

        sink.close().await.map_err(|e: tokio_postgres::Error| IngestorError::IngestionError(e.to_string()))?;
        Ok(())
    }
    ...
    async fn batch_ingest_stream(&self, stream: RecordStream, table_name: &str) -> Result<()> {
        ...
        let copy_query = format!("COPY \"{}\" (data) FROM STDIN (FORMAT CSV, HEADER FALSE)", table_name);
        let sink: CopyInSink<Bytes> = client.copy_in(&copy_query).await.map_err(|e| IngestorError::DatabaseError(e.to_string()))?;
        pin_mut!(sink);
    ...
    async fn ingest_as_blob(&self, data: DataReaderResult, table_name: &str) -> Result<()> {
        ...
        execute_with_retry(|| async {
            client.execute(&insert_query, &[&json_data])
                .await
                .map(|_| ())
