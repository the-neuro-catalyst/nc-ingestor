...
impl SqlSchemaBuilder {
    ...
    pub fn map_type(&self, nc_type: &DataType,) -> String {
        match (nc_type, &self.dialect,) {
            ...
            (DataType::Array(_,), SqlDialect::Sqlite,) => "TEXT".to_string(), /* Store as JSON
                                                                                * string */
            ...
            (DataType::Union(variants,), _dialect,) => {
                // If Union contains Null, we ignore it for type mapping (it just makes the column
                // nullable) We pick the first non-null type for mapping
